...
- **Все артефакты кода — ТОЛЬКО на английском**: комментарии, NatSpec, revert-строки, идентификаторы, сообщения коммитов, документация в `docs/`, вывод deploy-скриптов, тексты UI и лог-сообщения worker'а. Причина: репозиторий и живой дашборд смотрит международное жюри. (Рабочий язык диалога в сессиях — любой; в файлы попадает только английский.)
...
4. **Атомарность execute.** В цепочке `TruthGateBase.execute` → `_processAndEmitEvent` → бизнес-обработчики запрещены `try/catch` и низкоуровневые call'ы без проверки результата: любой сбой обязан откатывать транзакцию ЦЕЛИКОМ, включая пометку `processedQueries[queryId]`. Гарантия «queryId помечен ⟺ все эффекты применены» — иначе частично исполненный proof с сожжённым queryId становится невосстановимой дырой. Инвариант-тест: `test_invariant_queryIdMarkedIffEffectsApplied` (RepaymentBridge.t.sol).
